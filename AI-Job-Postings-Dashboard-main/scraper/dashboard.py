...
# Resolve data path relative to this file so the app can be run from any cwd
BASE_DIR = Path(__file__).resolve().parent
DATA_PATH = BASE_DIR / "data" / "skill_trends.csv"

# Load data with helpful error message if missing
if not DATA_PATH.exists():
    st.set_page_config(page_title="AI Job Skills Dashboard", layout="centered")
    st.title("📊 AI Job Skills Dashboard")
    st.error(f"Data file not found: {DATA_PATH}\n\nMake sure the CSV exists at this path or update the path in the script.")
    st.stop()
...
st.markdown("---")
st.caption("Built by Sonu Kumar  🔥 using Streamlit + Selenium")
